...
def load_data(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    ...
    for line in lines:
        ...
        if e1 not in unique_entities:
            unique_entities.append(e1)
        if e2 not in unique_entities:
            unique_entities.append(e2)
...
def load_data_test(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    ...
    print("number of unique_entities ->", len(unique_entities))
    return triples_data, (rows, cols, data), list(unique_entities)
def load_data_test2(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    with open(filename) as f:
        lines = f.readlines()
    triples_data = []
    rows, cols, data = [], [], []
    unique_entities = set()
    # 打乱
    idx = np.random.permutation(7416)[:100]
    # 不打乱
    # idx = [i for i in range(100)]
    for i, line in enumerate(lines):
        if i in idx:
...
def load_graph_dgi(config):
    ...
    nsadj = normalize2(sadj + sp.eye(sadj.shape[0]))
